...
class BackendProductController extends BackendController
{
    ...
    public function actionIndex()
    {
        ...
        if (null !== $catId = Yii::$app->request->get('parent_id')) {
            $dataProvider->query->leftJoin(
                Objects::getForClass(Product::className())->categories_table_name . ' cp',
    ...
    public function actionClone($id, $returnUrl = ['index'])
    {
        ...
        if ($newModel->save()) {
            $object = Objects::getForClass(get_class($newModel));
            // save categories
            $categoriesTableName = Objects::getForClass(Product::className())->categories_table_name;
            $query = new Query();
